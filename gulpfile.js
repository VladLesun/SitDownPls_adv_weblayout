...
const fontStyle = (done) => {
  ...
  fs.writeFile(srcFonts, "@use 'mixins';\r\n", cb);
    ...
    if (items) {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        if (c_fontname != fontname) {
          fs.appendFile(
            srcFonts,
            `@include mixins.font-face("${fontname}", "${fontname}", 400);\r\n`,
            cb
          );
        }
        c_fontname = fontname;
...
const images = () => {
  return src([
    ...
    `${paths.srcImgFolder}/hero/*.{jpg,jpeg,png,svg}`,
    `${paths.srcImgFolder}/high-rating/*.{jpg,jpeg,png,svg}`,
    `${paths.srcImgFolder}/oxford/*.{jpg,jpeg,png,svg}`,
    `${paths.srcImgFolder}/special-offers/*.{jpg,jpeg,png,svg}`,
    `${paths.srcImgFolder}/top-category/*.{jpg,jpeg,png,svg}`,
    `${paths.srcImgFolder}/useful/*.{jpg,jpeg,png,svg}`,
    `${paths.srcImgFolder}/contacts/*.{jpg,jpeg,png,svg}`,
    `${paths.srcImgFolder}/cooperation/*.{jpg,jpeg,png,svg}`,
  ])
    .pipe(
      gulpIf(
        isProd,
